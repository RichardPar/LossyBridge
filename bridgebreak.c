...
static int jitter_open(struct inode* inode, struct file* file);
static int jitter_show(struct seq_file* m, void* v);
static ssize_t jitter_write(struct file* file, const char* buf, size_t size, loff_t* ppos);

void do_jitter(void);

static const struct file_operations loss_fops = { .owner = THIS_MODULE,
                                                  .open = loss_open,
                                                  .write = loss_write,
                                                  .read = seq_read,
                                                  .llseek = seq_lseek,
...
static const struct file_operations jitter_fops = { .owner = THIS_MODULE,
                                                    .open = jitter_open,
                                                    .write = jitter_write,
                                                    .read = seq_read,
                                                    .llseek = seq_lseek,
...
{
    unsigned short ran;
    unsigned char interested = 0;

    if (in != NULL) {
        if (!strncmp(in->name, "eth1", 16)) {
            interested = 1;
        }
    }
...
void do_jitter()
{
    ...
    unsigned int actualLoop;
    unsigned short actual;

    get_random_bytes(&ran, 2);
    interval = (0xFFFF / jitter);

    actual = 0;

    for (actualLoop = 0; actualLoop < ran; actualLoop += interval) {
        actual++;
        mdelay(1);
    }
    // printk("Jitter is %d\r\n",actual);
...
static int jitter_show(struct seq_file* m, void* v)
{
    seq_printf(m, "Jitter %dmS", jitter);
    return 0;
}
...
static int jitter_open(struct inode* inode, struct file* file)
{
    return single_open(file, jitter_show, NULL);
}
...
int init_module()
{
    ...
    proc_create("jitter", 0, NULL, &jitter_fops);
